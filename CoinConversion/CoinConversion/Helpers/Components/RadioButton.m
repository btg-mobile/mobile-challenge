...
static const CFTimeInterval kDefaultAnimationDuration = 0.2;
...
- (NSArray *)allButtons {
    if ([_otherButtons count]) {
        NSMutableArray *buttons = [[NSMutableArray alloc] initWithCapacity:[_otherButtons count] + 1];
        for (NSValue *value in _otherButtons) {
            [buttons addObject:[value nonretainedObjectValue]];
        }
        [buttons addObject:self];
        return buttons;
    }
    return nil;
}
...
- (UIImage *)drawIconWithSelection:(BOOL)selected {
    ...
    UIColor *iconBackgroundColor = self.iconBackgroundColor ? self.iconBackgroundColor : [UIColor whiteColor];
