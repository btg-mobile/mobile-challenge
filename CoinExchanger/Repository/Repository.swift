...
class Repository {
    ...
    class func getCoins(completion: @escaping (GetCoinList?, Error?)->Void) {
        ...
        URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            do {
                let coins = try JSONDecoder().decode(GetCoinList.self, from: data)
    ...
    // MARK: Get Quotes
    ///  Fetch all coins from the server
    class func getQuotes(completion: @escaping (GetExchangeList?, Error?)->Void) {
        ...
        URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            do {
                let quotes = try JSONDecoder().decode(GetExchangeList.self, from: data)
